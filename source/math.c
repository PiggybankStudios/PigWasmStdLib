...
float powf(float value, float exponent)
{
	...
	{
		// Either (value < 0x1p-126 or inf or nan) or (exponent is 0 or inf or nan).
		if (predict_false(zeroinfnan(iy))) {
			...
			if ((2 * ix < 2 * 0x3f800000) == !(iy & 0x80000000))
				return 0.0f; // |value|<1 && exponent==inf or |value|>1 && exponent==-inf.
			return exponent * exponent;
		}
		if (predict_false(zeroinfnan(ix))) {
			float_t x2 = value * value;
		...
		// value and exponent are non-zero finite.
		if (ix & 0x80000000) {
			// Finite value < 0.
			int yint = checkint(iy);
			if (yint == 0)
				return __math_invalidf(value);
			if (yint == 1)
				sign_bias = SIGN_BIAS;
			ix &= 0x7fffffff;
		}
		if (ix < 0x00800000) {
			// Normalize subnormal value so exponent becomes negative.
			ix = asuint(value * 0x1p23f);
			ix &= 0x7fffffff;
			ix -= 23 << 23;
		}
	}
	double_t logx = log2_inline(ix);
	double_t ylogx = exponent * logx; /* cannot overflow, exponent is single prec.  */
	if (predict_false((asuint64(ylogx) >> 47 & 0xffff) >= asuint64(126.0 * POWF_SCALE) >> 47))
	{
		// |exponent*log(value)| >= 126.
