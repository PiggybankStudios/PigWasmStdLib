...
void* WasmMemoryAllocate(uint32_t numBytes)
{
	...
	{
		jsAbort("The WebAssembly module has run out of memory! WebAssembly only allows for 2GB of memory to be allocated per module");
		return nullptr;
	}
	else if (WasmMemoryNumPagesAllocated < numPagesNeeded)
	{
		jsGrowMemory(numPagesNeeded - WasmMemoryNumPagesAllocated);
